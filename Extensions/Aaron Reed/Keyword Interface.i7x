Version 6 of Keyword Interface by Aaron Reed begins here.
...
[Changelog:
  -- Version 6: Now emphasizes objects listed after opening an opaque container; omit release number in welcome message if it is 0.
...
A thing is either keyworded or keywordless. A thing is usually keyworded. Scenery is usually keywordless. [Keywordless things are not automatically highlighted in room descriptions.]
...
not-a-verb-I-recognise	"That's neither a verb I [if the American dialect option is active]recognize[else]recognise[end if] nor a keyword you can use right now."
keyword-introduction	"As you read [story title], you'll see certain emphasized keywords in the prose. Type any keyword to advance the story. [if object keyword highlighting is true]You can type an emphasized [o]object[x] keyword to examine that item more closely. [end if][if direction keyword highlighting is true]An emphasized [d]direction[x] indicates that typing that word will move you that direction or towards that distant scenery. [end if][if topic keyword highlighting is true]An emphasized word in [t]conversation[x] means typing that word will steer the conversation towards that topic. [end if]If the keyword[if the number of active keyword systems > 1]s are[else] is[end if] not distinct from the surrounding text, type KEYWORDS to adjust them for your system."
welcome-message	"Welcome to [o][story title][x][if release number > 0], release [release number][end if].[paragraph break]If no words above were colored or emphasized, press K now.[paragraph break]Press [t]N[x] to begin from the beginning or [t]R[x] to restore an existing story."
keyword-setting-instructions	"[story title] makes use of emphasized keywords. It is [if keywords required is true]necessary[else]recommended[end if] that your interpreter program correctly displays them with some form of emphasis. Type a number below to cycle through display options until you find one that shows clearly on your system."
keyword-instructions-object	"Emphasized [o]object[x] keywords currently look like [o]this[x]."
keyword-instructions-direction	"Emphasized [d]exit[x] keywords currently look like [d]this[x]."
keyword-instructions-topic	"Emphasized [t]topic[x] keywords currently look like [t]this[x]."
keyword-instructions-parser	"[as the parser]Messages from the parser currently display like this[as normal]."
keyword-instructions-disabled	"**A warning: The emphasized keywords are integral to [story title]'s design. It may be difficult or impossible to know how to continue if emphasis is not visible.**"
keyword-instructions-distinct	"**A warning: it may at times be useful to be able to differentiate between the various kinds of keyword.**"
...
screen-reader-activated	"Screen reader mode has been activated. [story title] uses emphasized keywords to indicate important words you may type to advance the story. Your reader software may put a recognizable vocal stress on emphasized words like [o]this[x] one. If it doesn't, you can type [o]things[x] from any prompt for a list of nearby objects, or type [d]exits[x] for a list of nearby directions. Type [o]keywords[x] to adjust the style of keywords, or type screen reader with no space to toggle this mode on or off."
...
When object keyword highlighting is set to true, the extension will automatically highlight objects listed in bulk when looking, opening or taking inventory, as well as objects manually invoked in room descriptions if their names are bracketed.
...
Object highlighting, when activated, highlights keywords when looking, opening or taking inventory. If you want to disable one of these you can use code like the following:
...
Since there are so many vastly different conversation systems, support for highlighting conversation topics is only vestigial. You can turn it on via "topic keyword highlighting" and invoke it with the "[t]topic[x]" wrappers, but everything else will have to be done by hand to match your own conversation system.

Section: Parser Messages

In addition, the extension provides support to print messages output by the parser in a different style than normal game text. This may be useful to authors who wish to differentiate error messages ("I didn't understand the first part of that.") from in-game admonishments ("You wouldn't be caught dead wearing that.")
...
If your game defines any of its own parser error messages, you should wrap them in the "as the parser" and "as normal" say statements:
...
As noted above, authors can enable or disable keyword highlighting for any component. Players also have control over keywording by using the KEYWORD verb, which brings up a menu with flexible options, customised to which keywords are enabled and with instructions for activating a screen reader mode for blind players.
...
You can print some stock explanatory text (customized to which keywords are enabled) by using the following line, perhaps in the carry out rule for an "about" command:
...
To change the Glulx colors, you can adjust g-color1 and g-color2 like this:

	Table of Common Color Values (amended)
	glulx color value	assigned number   
	g-color1	15645627

See the documentation for "Glulx Text Effects" by Emily Short for more info on Glulx colors.

All of the messages the extension prints are customizeable by amending the Table of Keyword Interface messages, eg:

	Table of Keyword Interface messages (amended)
	KI message			KI output
	keyword-setting-instructions		"[story title] makes use of emphasized keywords. It is important that your interpreter program correctly displays them with some form of emphasis. Type a number below to cycle through display options until you find one that shows clearly on your system."

The messages and how they currently begin are as follows:

	not-a-verb-I-recognise	"That's neither a verb I recognize nor a keyword you can use right now."
	keyword-introduction	"As you read [story title], you'll see certain emphasized keywords in the prose...."
	welcome-message	"Welcome to [o][story title][x], release [release number].[paragraph break]If no words above were colored or emphasized, press K now...."
	keyword-setting-instructions	"[story title] makes use of emphasized keywords. It is important that your interpreter program correctly displays them..."
	keyword-instructions-object	"Emphasized [o]object[x] keywords currently look like [o]this[x]."
	keyword-instructions-direction	"Emphasized [d]exit[x] keywords currently look like [d]this[x]."
	keyword-instructions-topic	"Emphasized [t]topic[x] keywords currently look like [t]this[x]."
	keyword-instructions-parser	"[as the parser]Messages from the parser currently display like this[as normal]."
	keyword-instructions-disabled	"**A warning: The emphasized keywords are integral to [story title]'s design..."
	keyword-instructions-distinct	"**A warning: it may at times be useful to be able to differentiate between the various kinds of keyword.**"
	keyword-instructions-end	"Press 0 when you're finished here..."
	screen-reader-activated	"Screen reader mode has been activated..."
	screen-reader-deactivated	"Screen reader mode has been deactivated."
...
The flexibility of Keyword Interface means it can be used for multiple purposes. Here we strip away and focus some of its functionality to simply print compass directions in bold.
	...
	*: "Sense of Direction"
	...
	First when play begins: now object keyword highlighting is false; now the style of direction-word is keyword-bold-style.
...
We'll create a slightly longer scenario with several locations and a simple puzzle, to demonstrate the full potential of Keyword Interface. This game has "keywords required" set to true, so it begins with a menu that helps players calibrate keywords.
...
We implement an extremely simple conversation system to demonstrate emphasized topic words. Note that a real conversation system would need to be much more robust and elaborate. We'll also create a variation on the exits verb to show our compassless exits.
	...
	*: "The Gnome's Holiday"
	...
	First when play begins: now topic keyword highlighting is true; now parser highlighting is true; now keywords required is true.  
