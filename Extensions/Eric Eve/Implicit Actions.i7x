Version 11 of Implicit Actions by Eric Eve begins here.
...
To finish the implicit action with participle (partc - some text) infinitive (inf - some text) object (obj - an indexed text) and condition (cond - a truth state):
  ...
  if parenthesize implicit actions is true then
     change the implicit action report to "[implicit action report][if number of characters in implicit action report > 0], then [end if][if cond is true][partc][otherwise]trying to [inf][end if] [obj]";
  otherwise change the implicit action report to "[implicit action report][if number of characters in implicit action report > 0], then [end if][if cond is false]try to [end if][inf] [obj]";
  end if;
...
In this example we define a new action (kicking) and a new implicit action (dropping something). It's a bit dificult to kick something you're holding, so we make the kick action carry out an implicit drop if the thing to be kicked is currently carried.
	...
	*: "Football Practice"
	...
	The ball is in the Football Pitch. It can be in-play or off-pitch. It is in-play.
