Version 6 of Conversation Framework by Eric Eve begins here.
...
Before implicit-conversing when the current interlocutor is yourself (this is the implicit-conversing needs current interlocutor rule):
let np be the number of visible people who are not the player;
if np is 0 then say "There's no one here to talk to.";
if np > 1 then say "You need to specify who you want to speak to.";
...
Before implicit-conversing when the current interlocutor is not visible and the current interlocutor is not yourself  (this is the can't converse with absent interlocutor rule):
...
Check hailing (this is the check what's being hailed rule):
  if the current interlocutor is a visible person and the current interlocutor is not the player then
...
To reset the interlocutor:
  change the current interlocutor to yourself. [a random person who is not a person]
...
Every thing in the game is either known or unknown, but this is not a property that can be seen directly. A thing is known either if the player character has seen it or if it familiar: the familiar/unfamiliar property is one we can set directly (with 'change' or 'now').

Since the player might well to come to know something in the course of conversation, we provide a convenient 'to say' phrase ('make something known') which can be used to make an object familiar (and hence known) within a piece of text that's being said, e.g.:
...
This allows the player to refer to the lost wallet in subsequent conversational commands (whereas previously ASK BOB ABOUT WALLET would be treated as asking him about some unknown topic).
...
The extension Conversation Responses provides a framework for handling conversational command through rules like 'Response of Bob when asked about Mary'.
